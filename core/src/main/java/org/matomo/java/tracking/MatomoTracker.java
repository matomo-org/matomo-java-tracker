...
public class MatomoTracker implements AutoCloseable {
  ...
  /**
   * Send a request asynchronously via HTTP GET and specify a callback that gets executed when the
   * response arrives.
   *
   * <p>Use this method if you want to send a single request. If you want to send multiple requests
  ...
  public <T> CompletableFuture<T> sendRequestAsync(
      @NonNull MatomoRequest request, @NonNull Function<MatomoRequest, T> callback) {
    if (trackerConfiguration.isEnabled()) {
      applyGoalIdAndCheckSiteId(request);
   ...
   * individual requests.
   *
   * <p>Specify the AuthToken if parameters that require an auth token is used. If you want to send
  ...
  /**
   * Send multiple requests in a single HTTP call. More efficient than sending several individual
   * requests. Specify the AuthToken if parameters that require an auth token is used.
   *
   * @param requests the requests to send
   * @param authToken specify if any of the parameters use require AuthToken, if null the default
   *     auth token from the request or the tracker configuration is used
   ...
   * @deprecated Please set the auth token in the tracker configuration or the requests directly and
   *     use {@link CompletableFuture#thenAccept(Consumer)} instead for the callback.
   */
  @Deprecated
  public CompletableFuture<Void> sendBulkRequestAsync(
      @NonNull Collection<? extends MatomoRequest> requests,
   ...
   */
  public CompletableFuture<Void> sendBulkRequestAsync(
      @NonNull Collection<? extends MatomoRequest> requests, @Nullable Consumer<Void> callback) {
   ...
   * @return completable future to let you know when the request is done
   * @deprecated Please set the auth token in the tracker configuration or the requests directly and
   *     use {@link #sendBulkRequestAsync(Collection)} instead.
   */
  public CompletableFuture<Void> sendBulkRequestAsync(

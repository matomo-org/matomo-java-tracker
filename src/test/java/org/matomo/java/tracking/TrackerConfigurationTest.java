...
class TrackerConfigurationTest {
  ...
  void validateDoesNotFailIfDefaultAuthTokenIsNull() throws Exception {
    TrackerConfiguration trackerConfiguration = TrackerConfiguration.builder().apiEndpoint(
        URI.create("https://matomo.example/matomo.php")).defaultSiteId(1).defaultAuthToken(null)
      .build();
  ...
  void validateFailsIfDefaultAuthTokenIsEmpty() throws Exception {
    TrackerConfiguration trackerConfiguration = TrackerConfiguration.builder().apiEndpoint(
        URI.create("https://matomo.example/matomo.php")).defaultSiteId(1).defaultAuthToken("")
      .build();

    assertThatThrownBy(trackerConfiguration::validate).isInstanceOf(IllegalArgumentException.class)
